...
export class Tab4Page implements OnInit {
   ...
   selectPlace(place, index) {
      ...
      if (this.places[this.selectedPlaceIndex].photos) {
         let photo_reference = this.places[this.selectedPlaceIndex].photos[0].photo_reference;
         this.googlePlacesService.getPlacePhotoByReference(photo_reference).subscribe(photo => {
            console.log(photo);
             this.selectedPlacePhoto = URL.createObjectURL(photo);
         })
      }
   }
